...
const reshapeLineItem = (lineItem: MedusaLineItem): CartItem => {
  ...
  const cost = {
    totalAmount: {
      ...
      currencyCode:
        lineItem.variant?.prices?.[0]?.currency_code.toUpperCase() || "EUR",
    },
  };
...
const reshapeProduct = (product: MedusaProduct): Product => {
  ...
  const featuredImage = {
    ...
    altText: product.thumbnail
      ? `${product.title} - ${featuredImageFilename}`
      : "",
  };
...
const reshapeCategory = (category: ProductCategory): ProductCollection => {
  const description =
    category.description || category.metadata?.description?.toString() || "";
  const seo = {
    title: category?.metadata?.seo_title?.toString() || category.name || "",
    description:
      category?.metadata?.seo_description?.toString() ||
